...
import type { TelegramMessage, TelegramChat, TelegramResponse } from './types.js';

export type { TelegramMessage, TelegramChat, TelegramResponse };
...
async function telegramRequest<T>(
  ...
  if (!response.ok) throw new HttpError(response.description || 'Telegram API error');
  return response.result;
}

export async function sendMessage(
  chatId: string | number, text: string,
  options: { parseMode?: 'HTML' | 'Markdown' | 'MarkdownV2'; disableNotification?: boolean; replyToMessageId?: number } = {}
...
export async function sendNotification(text: string, options: { silent?: boolean } = {}): Promise<TelegramMessage> {
  const credentials = getTelegramCredentials();
  if (!credentials?.default_chat_id) throw new HttpError('No default_chat_id configured');
  return sendMessage(credentials.default_chat_id, text, { disableNotification: options.silent });
}

export async function sendDocument(chatId: string | number, filePath: string, caption?: string): Promise<TelegramMessage> {
...
export async function sendPhoto(chatId: string | number, photoUrl: string, caption?: string): Promise<TelegramMessage> {
  const params: Record<string, unknown> = { chat_id: chatId, photo: photoUrl };
  if (caption) params.caption = caption;
  return telegramRequest<TelegramMessage>('sendPhoto', params);
}

export async function editMessage(chatId: string | number, messageId: number, text: string, parseMode?: 'HTML' | 'Markdown' | 'MarkdownV2'): Promise<TelegramMessage> {
...
export async function getUpdates(
  ...
  const params: Record<string, unknown> = { limit: options.limit || 10, timeout: options.timeout || 0 };
...
export async function sendMessageWithButtons(
  ...
  return telegramRequest<TelegramMessage>('sendMessage', { chat_id: chatId, text, reply_markup: { inline_keyboard: buttons } });
}

export async function forwardMessage(chatId: string | number, fromChatId: string | number, messageId: number): Promise<TelegramMessage> {
  return telegramRequest<TelegramMessage>('forwardMessage', { chat_id: chatId, from_chat_id: fromChatId, message_id: messageId });
}

export async function pinMessage(chatId: string | number, messageId: number, silent = false): Promise<boolean> {
  await telegramRequest<boolean>('pinChatMessage', { chat_id: chatId, message_id: messageId, disable_notification: silent });
  return true;
}

export function isAuthenticated(): boolean { return getTelegramCredentials() !== null; }

export function getDefaultChatId(): string | undefined { return getTelegramCredentials()?.default_chat_id; }
