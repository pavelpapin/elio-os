...
|--------|------|-------------|
| agent_folder | directory | /root/.claude/workflows/{name}/ |
...
**Actions:**
1. Создать директорию `/root/.claude/workflows/{name}/`
...
**⛔ GATE CHECK:**
- [ ] Директория `/root/.claude/workflows/{name}/` существует
- [ ] AGENT.md существует
...
Файлы:
- /root/.claude/workflows/competitor-monitor/AGENT.md
- /root/.claude/workflows/competitor-monitor/config.json
...
```json
{
  ...
  "templates_dir": "/root/.claude/workflows/_templates",
  "notifications": {
    "onStart": true,
    "onStageChange": true,
    "onComplete": true,
